  ...
  async def run(self, input_path: str, output_path: str):
    ...
    try:
      ...
      if isinstance(input_path, (list, tuple)):
          all_numbers = []
          for p in input_path:
              nums = await self.read_from_afs(p)
...
async def main():
  ...
  if "--input" in sys.argv:
      n = sys.argv.index("--input") + 1
      input_paths = []
      while n < len(sys.argv) and not sys.argv[n].startswith("--"):
          input_paths.append(sys.argv[n])
          n += 1
  ...
  if "--workers" in sys.argv:
    n = sys.argv.index("--workers") + 1
    if n < len(sys.argv) and sys.argv[n].isdigit():
        NUM_WORKERS = int(sys.argv[n])
    else:
        sys.exit(1)
  else:
    NUM_WORKERS = 4
  ...
  print(f"  Input AFS Path(s): {', '.join(input_paths)}")
  ...
  try:
    await coordinator.run(input_path=input_paths, output_path=output_path)
